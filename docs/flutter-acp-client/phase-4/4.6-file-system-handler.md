# Задача 4.6 — File System Handler

## Цель

Реализовать обработчик запросов файловой системы от ACP агента. Это реализация `FileSystemService` интерфейса.

---

## Шаг 1 — File System Handler

Создай файл `lib/infrastructure/handlers/file_system_handler.dart`:
...
- [ ] `FileSystemHandler` реализует `FileSystemService`
- [ ] Поддержка частичного чтения (line + limit параметры)
- [ ] Создание директорий при записи
- [ ] Ошибки оборачиваются в `FileSystemFailure`
- [ ] `FileSystemHandler` зарегистрирован в `SessionModule` как singleton `FileSystemService`
