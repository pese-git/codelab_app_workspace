# Задача 4.7 — Terminal Handler

## Цель

Реализовать обработчик терминальных запросов от ACP агента через `dart:io`.

---

## Шаг 1 — Terminal Process Registry

Создай файл `lib/infrastructure/handlers/terminal_handler.dart`:
...
class TerminalHandler implements TerminalService {
  ...
  Future<String> create({
    ...
    try {
      ...
      final int byteLimit = outputByteLimit ?? 1024 * 1024; // 1MB default
...
- [ ] `TerminalHandler` реализует `TerminalService`
- [ ] Создание процесса через `Process.start()`
- [ ] Сбор stdout/stderr в буфер с ограничением по размеру
- [ ] `release()` корректно завершает подписки и процесс
- [ ] Ошибки оборачиваются в `TerminalFailure`
- [ ] `TerminalHandler` зарегистрирован в `SessionModule` как singleton `TerminalService`
